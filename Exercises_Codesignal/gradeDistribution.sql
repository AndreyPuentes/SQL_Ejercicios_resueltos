...
best grade comes from Option 3, sorted based on the first 3 characters of their name. If the first 3 characters 
of two names are the same, then the student with the lower ID value comes first. */
...
BEGIN
	SELECT G.name, G.ID
    FROM Grades AS G 
    WHERE G.Final > (G.Midterm1*0.5 + G.Midterm2*0.5) AND G.Final > (G.Midterm1*0.25 + G.Midterm2*0.25 +G.Final*0.5) 
    ORDER BY Substring(G.name, 1, 3);
END;